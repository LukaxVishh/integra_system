...
const OrientadorTableEditor: React.FC<Props> = ({ buttonId, onClose }) => {
  ...
  const canManage = hasAnyClaim(["CoCreateOri", "CoUpdateOri", "CanManageAll"]);
  ...
  // Carrega tabela
  useEffect(() => {
    fetch(`http://localhost:5000/cooperativismo/orientador/buttons/${buttonId}/table`, {
      credentials: "include",
    })
  ...
  const saveTable = async () => {
    await fetch(`http://localhost:5000/cooperativismo/orientador/buttons/${buttonId}/table`, {
      method: "POST",
      credentials: "include",
