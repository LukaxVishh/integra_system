// CooperativismoInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import CooperativismoPostCard from "./CooperativismoPostCard";
import CreateCooperativismoPost from "./CreateCooperativismoPost";
...
const CooperativismoInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/cooperativismo?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ Cooperativismo posts:", data);
      ...
      console.error("Erro ao buscar posts do Cooperativismo:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <CreateCooperativismoPost />

      {/* Teste: se nada mostrar, teste com <pre>{JSON.stringify(posts)}</pre> */}
      {posts.map((post) => (
        <CooperativismoPostCard key={post.id} {...post} />
      ))}
...
export default CooperativismoInfinitePosts;
