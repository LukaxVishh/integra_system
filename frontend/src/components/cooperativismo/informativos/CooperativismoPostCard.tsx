...
interface CooperativismoPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const CooperativismoPostCard: React.FC<CooperativismoPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/cooperativismo/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default CooperativismoPostCard;
