...
import NegociosColaboradorModal from "./NegociosColaboradorModal";
...
const NegociosColaboradores: React.FC = () => {
  ...
  const isGerenteNegocios = hasClaim("GerenteNegocios");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/negocios/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerenteNegocios || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente Negocios encontrado.</p>;
    ...
    <div className="tree-container">
      ...
      {editingColaborador && (
        <NegociosColaboradorModal
          ...
          canEdit={
            isAdmin || isGerenteNegocios || currentUser?.id === editingColaborador.id
          }
...
export default NegociosColaboradores;
