...
interface NegociosPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const NegociosPostCard: React.FC<NegociosPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/Negocios/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default NegociosPostCard;
