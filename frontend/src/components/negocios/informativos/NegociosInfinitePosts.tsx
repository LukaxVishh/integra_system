// NegociosInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import NegociosPostCard from "./NegociosPostCard";
import CreateNegociosPost from "./CreatenegociosPost";
...
const NegociosInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/Negocios?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ Negocios posts:", data);
      ...
      console.error("Erro ao buscar posts do Negocios:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      {posts.map((post) => (
        <NegociosPostCard key={post.id} {...post} />
      ))}
...
export default NegociosInfinitePosts;
