...
import OperacoesColaboradorModal from "./OperacoesColaboradorModal";
...
const OperacoesColaboradores: React.FC = () => {
  ...
  const isGerenteOperacoes = hasClaim("GerenteOperacoes");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/operacoes/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerenteOperacoes || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente Operacoes encontrado.</p>;
    ...
    <div className="tree-container">
      ...
      {editingColaborador && (
        <OperacoesColaboradorModal
          ...
          canEdit={
            isAdmin || isGerenteOperacoes || currentUser?.id === editingColaborador.id
          }
...
export default OperacoesColaboradores;
