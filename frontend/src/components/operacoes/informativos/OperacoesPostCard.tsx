...
interface OperacoesPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const OperacoesPostCard: React.FC<OperacoesPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/operacoes/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default OperacoesPostCard;
