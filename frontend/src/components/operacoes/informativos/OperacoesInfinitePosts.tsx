// OperacoesInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import OperacoesPostCard from "./OperacoesPostCard";
import CreateOperacoesPost from "./CreateOperacoesPost";
...
const OperacoesInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/operacoes?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ Operacoes posts:", data);
      ...
      console.error("Erro ao buscar posts de Operacoes:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      {posts.map((post) => (
        <OperacoesPostCard key={post.id} {...post} />
      ))}
...
export default OperacoesInfinitePosts;
