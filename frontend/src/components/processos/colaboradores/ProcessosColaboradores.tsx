...
import ProcessosColaboradorModal from "./ProcessosColaboradorModal";
...
const ProcessosColaboradores: React.FC = () => {
  ...
  const isGerenteProcessos = hasClaim("GerenteProcessos");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/processos/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerenteProcessos || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente Processos encontrado.</p>;
    ...
    <div className="tree-container">
      ...
      {editingColaborador && (
        <ProcessosColaboradorModal
          ...
          canEdit={
            isAdmin || isGerenteProcessos || currentUser?.id === editingColaborador.id
          }
...
export default ProcessosColaboradores;
