...
interface ProcessosPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const ProcessosPostCard: React.FC<ProcessosPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/processos/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default ProcessosPostCard;
