// ProcessosInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import ProcessosPostCard from "./ProcessosPostCard";
import CreateProcessosPost from "./CreateProcessosPost";
...
const ProcessosInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/processos?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ Processos posts:", data);
      ...
      console.error("Erro ao buscar posts do Processos:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      {posts.map((post) => (
        <ProcessosPostCard key={post.id} {...post} />
      ))}
...
export default ProcessosInfinitePosts;
