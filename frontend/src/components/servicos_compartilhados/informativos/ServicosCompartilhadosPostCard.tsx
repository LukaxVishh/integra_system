...
interface ServicosCompartilhadosPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const ServicosCompartilhadosPostCard: React.FC<ServicosCompartilhadosPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/servicos-compartilhados/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default ServicosCompartilhadosPostCard;
