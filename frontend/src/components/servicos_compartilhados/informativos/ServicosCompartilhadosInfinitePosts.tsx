// ServicosCompartilhadosInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import ServicosCompartilhadosPostCard from "./ServicosCompartilhadosPostCard";
import CreateServicosCompartilhadosPost from "./CreateServicosCompartilhadosPost";
...
const ServicosCompartilhadosInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/servicos-compartilhados?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ ServicosCompartilhados posts:", data);
      ...
      console.error("Erro ao buscar posts do Servicos Compartilhados:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <CreateServicosCompartilhadosPost />

      {/* Teste: se nada mostrar, teste com <pre>{JSON.stringify(posts)}</pre> */}
      {posts.map((post) => (
        <ServicosCompartilhadosPostCard key={post.id} {...post} />
      ))}
...
export default ServicosCompartilhadosInfinitePosts;
