...
import ServicosCompartilhadosColaboradorModal from "./ServicosCompartilhadosColaboradorModal";
...
const ServicosCompartilhadosColaboradores: React.FC = () => {
  ...
  const isGerenteServicosCompartilhados = hasClaim("GerenteServicosCompartilhados");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/servicos-compartilhados/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerenteServicosCompartilhados || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente Servicos Compartilhados encontrado.</p>;
  ...
  return (
    <div className="tree-container">
      ...
      {editingColaborador && (
        <ServicosCompartilhadosColaboradorModal
          ...
          canEdit={
            isAdmin || isGerenteServicosCompartilhados || currentUser?.id === editingColaborador.id
          }
...
export default ServicosCompartilhadosColaboradores;
