...
const visibilityOptions = ["Cooperativa", "Centro Administrativo", "Agência"];
        ...
        <div className="absolute right-0 top-full mt-2 bg-white border border-[#E6F4EA] rounded shadow z-20">
          {visibilityOptions.map(option => (
            <button
              ...
              className={`block w-full text-left px-4 py-2 hover:bg-[#F1F8F5] ${
                visibility === option ? "font-bold text-[#0F9D58]" : ""
              }`}
            >
              {option}
            </button>
      ...
      {[
        ...
        <button
          ...
          className={`px-2 py-1 rounded ${
            isActive(cmd) ? "bg-[#E6F4EA] text-[#0F9D58]" : "hover:bg-[#F1F8F5]"
          }`}
        >
          {label}
        </button>
