...
const OrientadorTableEditor: React.FC<Props> = ({ buttonId, onClose }) => {
  ...
  const canManage = hasAnyClaim(["PcCreateOri", "PcUpdateOri", "CanManageAll"]);
  ...
  // Carrega tabela
  useEffect(() => {
    fetch(`http://localhost:5000/pessoas-cultura/orientador/buttons/${buttonId}/table`, {
      credentials: "include",
    })
  ...
  const saveTable = async () => {
    await fetch(`http://localhost:5000/pessoas-cultura/orientador/buttons/${buttonId}/table`, {
      method: "POST",
      credentials: "include",
