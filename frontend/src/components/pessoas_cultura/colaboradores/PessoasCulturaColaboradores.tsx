...
import PessoasCulturaColaboradorModal from "./PessoasCulturaColaboradorModal";
...
const PessoasCulturaColaboradores: React.FC = () => {
  ...
  const isGerentePessoasCultura = hasClaim("GerentePessoasCultura");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/pessoas-cultura/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerentePessoasCultura || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente de Pessoas e Cultura encontrado.</p>;
  ...
  return (
    <div className="tree-container">
      ...
      {editingColaborador && (
        <PessoasCulturaColaboradorModal
          ...
          canEdit={
            isAdmin || isGerentePessoasCultura || currentUser?.id === editingColaborador.id
          }
...
export default PessoasCulturaColaboradores;
