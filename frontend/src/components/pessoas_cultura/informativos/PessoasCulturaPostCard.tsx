...
interface PessoasCulturaPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const PessoasCulturaPostCard: React.FC<PessoasCulturaPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/pessoas-cultura/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default PessoasCulturaPostCard;
