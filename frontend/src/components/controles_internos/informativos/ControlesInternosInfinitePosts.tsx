// ControlesInternosInfinitePosts.tsx - Correto e comentado
import React, { useState, useEffect, useRef } from "react";
import ControlesInternosPostCard from "./ControlesInternosPostCard";
import CreateControlesInternosPost from "./CreateControlesInternosPost";
...
const ControlesInternosInfinitePosts: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/controles-internos?page=${page}&pageSize=${pageSize}`, {
        credentials: "include",
      });
      const data = await res.json();
      console.log("✅ ControlesInternos posts:", data);
      ...
      console.error("Erro ao buscar posts do Controles Internos:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <CreateControlesInternosPost />

      {/* Teste: se nada mostrar, teste com <pre>{JSON.stringify(posts)}</pre> */}
      {posts.map((post) => (
        <ControlesInternosPostCard key={post.id} {...post} />
      ))}
...
export default ControlesInternosInfinitePosts;
