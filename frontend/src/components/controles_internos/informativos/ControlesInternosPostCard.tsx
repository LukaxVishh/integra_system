...
interface ControlesInternosPostCardProps {
  id: number;
  authorName: string;
  authorCargo: string;
  content: string;
...
const ControlesInternosPostCard: React.FC<ControlesInternosPostCardProps> = ({
  id,
  authorName,
  authorCargo,
  content,
  ...
  const handleSaveEdit = async () => {
    setSavingEdit(true);
    try {
      const res = await fetch(`http://localhost:5000/controles-internos/${id}`, {
        method: "PUT",
        credentials: "include",
...
export default ControlesInternosPostCard;
