...
import ControlesInternosColaboradorModal from "./ControlesInternosColaboradorModal";
...
const ControlesInternosColaboradores: React.FC = () => {
  ...
  const isGerenteControlesInternos = hasClaim("GerenteControlesInternos");

  const reloadOrganograma = useCallback(async () => {
    try {
      const res = await fetch("http://localhost:5000/controles-internos/organograma", {
        credentials: "include",
      });
  ...
  const canEdit = (id: string) =>
    isAdmin || isGerenteControlesInternos || currentUser?.id === id;

  if (loading) return <p className="animate-pulse text-gray-500">Carregando colaboradores...</p>;
  if (!organograma) return <p>Nenhum gerente Controles Internos encontrado.</p>;
  ...
  return (
    <div className="tree-container">
      ...
      {editingColaborador && (
        <ControlesInternosColaboradorModal
          ...
          canEdit={
            isAdmin || isGerenteControlesInternos || currentUser?.id === editingColaborador.id
          }
...
export default ControlesInternosColaboradores;
