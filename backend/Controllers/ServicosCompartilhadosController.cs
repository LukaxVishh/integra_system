...
{
[Authorize]
    [Route("servicos-compartilhados")]
    [ApiController]
    public class ServicosCompartilhadosController : Controller
    {
        private readonly AppDbContext _context;
        private readonly IWebHostEnvironment _env;
        private readonly UserManager<IdentityUser> _userManager;

        public ServicosCompartilhadosController(AppDbContext context, IWebHostEnvironment env, UserManager<IdentityUser> userManager)
        {
            _context = context;
            _env = env;
        ...
        // === CREATE ===
        [HttpPost]
        public async Task<IActionResult> CreateServicosCompartilhadosPost([FromForm] PostCreateDto dto)
        {
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canCreate = User.HasClaim("ScCreatePost", "true");
            ...
            var post = new ServicosCompartilhadosPost
            {
                AuthorId = user.Id,
            ...
            _context.ServicosCompartilhadosPosts.Add(post);
        ...
        // === GET ===
        [HttpGet]
        public async Task<IActionResult> GetServicosCompartilhadosPosts([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
        {
            var user = await _userManager.GetUserAsync(User);
            ...
            var query = _context.ServicosCompartilhadosPosts
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateServicosCompartilhadosPost(int id, [FromBody] PostUpdateDto dto)
        {
            var post = await _context.ServicosCompartilhadosPosts.FindAsync(id);
            ...
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canUpdate = User.HasClaim("ScUpdatePost", "true");
            ...
            return Ok(new { message = "ServicosCompartilhadosPost atualizado com sucesso.", post });
        }

        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteServicosCompartilhadosPost(int id)
        {
            var post = await _context.ServicosCompartilhadosPosts.FindAsync(id);
            if (post == null) return NotFound();
            ...
            var isSupervisor = author?.SupervisorId == user.Id;
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canDelete = User.HasClaim("ScDeletePost", "true");
            ...
            _context.ServicosCompartilhadosPosts.Remove(post);
            await _context.SaveChangesAsync();

            return Ok(new { message = "ServicosCompartilhadosPost removido com sucesso." });
        }

        [HttpGet("orientador/buttons")]
        public async Task<IActionResult> GetButtons()
        {
            var buttons = await _context.ServicosCompartilhadosOrientadorButtons
                .OrderBy(b => b.Order)
                .ToListAsync();
            return Ok(buttons);
        }
        ...
        {
            if (!User.HasClaim("ScCreateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var maxOrder = await _context.ServicosCompartilhadosOrientadorButtons.MaxAsync(b => (int?)b.Order) ?? 0;

            var button = new ServicosCompartilhadosOrientadorButton
            {
                Text = dto.Text,
                Color = dto.Color,
                Bold = dto.Bold,
            ...
            _context.ServicosCompartilhadosOrientadorButtons.Add(button);
            await _context.SaveChangesAsync();
        ...
        {
            if (!User.HasClaim("ScUpdateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.ServicosCompartilhadosOrientadorButtons.FindAsync(id);
        ...
        {
            if (!User.HasClaim("ScDeleteOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.ServicosCompartilhadosOrientadorButtons.FindAsync(id);
            if (button == null) return NotFound();

            _context.ServicosCompartilhadosOrientadorButtons.Remove(button);
            await _context.SaveChangesAsync();
            return Ok();
        }
        ...
        public async Task<IActionResult> Reorder([FromBody] List<int> orderedIds)
        {
            ...
            var buttons = await _context.ServicosCompartilhadosOrientadorButtons.ToListAsync();
        ...
        {
            var table = await _context.ServicosCompartilhadosOrientadorTables
        ...
        {
            if (!User.HasClaim("ScCreateOri", "true") &&
                !User.HasClaim("ScUpdateOri", "true") &&
                !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var existing = await _context.ServicosCompartilhadosOrientadorTables
                .FirstOrDefaultAsync(t => t.ButtonId == buttonId);

            if (existing == null)
            {
                var newTable = new ServicosCompartilhadosOrientadorTable
                {
                    ButtonId = buttonId,
                    DataJson = dto.DataJson
                };
                _context.ServicosCompartilhadosOrientadorTables.Add(newTable);
            }
            else
            {
        ...
        [HttpGet("organograma")]
        public async Task<IActionResult> GetServicosCompartilhadosOrganograma()
        {
            var allUsers = await _userManager.Users.ToListAsync();
            var gerentesServicosCompartilhados = new List<object>();

            foreach (var user in allUsers)
            {
                var userClaims = await _userManager.GetClaimsAsync(user);

                if (userClaims.Any(c => c.Type == "GerenteServicosCompartilhados" && c.Value == "true"))
                {
                    var colaborador = await _context.Colaboradores
                        .FirstOrDefaultAsync(c => c.Email.ToLower() == user.Email.ToLower());

                    // ⚡ Gerente: atividades, tags, historico
                    var gerenteAtividades = await _context.ServicosCompartilhadosColaboradorAtividades
                    ...
                    var subordinados = rawSubordinados
                        .Select(c =>
                        {
                            ...
                            var atividades = _context.ServicosCompartilhadosColaboradorAtividades
                    ...
                    gerentesServicosCompartilhados.Add(new
                    {
                        id = user.Id,
            ...
            return Ok(gerentesServicosCompartilhados);
        }


        [HttpPost("colaboradores/atividades")]
        public async Task<IActionResult> CriarAtividade([FromBody] ServicosCompartilhadosColaboradorAtividade dto)
        {
            var user = await _userManager.GetUserAsync(User);
            var hasPower = User.HasClaim("GerenteServicosCompartilhados", "true") || User.HasClaim("CanManageAll", "true");
            ...
            _context.ServicosCompartilhadosColaboradorAtividades.Add(dto);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> AtualizarAtividade(int id, [FromBody] ServicosCompartilhadosColaboradorAtividade update)
        {
            var atividade = await _context.ServicosCompartilhadosColaboradorAtividades.FindAsync(id);
        ...
        public async Task<IActionResult> DeletarAtividade(int id)
        {
            ...
            _context.ServicosCompartilhadosColaboradorAtividades.Remove(atividade);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> ListarAtividadesPorColaborador(string email)
        {
            ...
            var atividades = await _context.ServicosCompartilhadosColaboradorAtividades
