...
namespace backend.Controllers
{
    ...
    public class OperacoesController : Controller
    {
        ...
        public OperacoesController(AppDbContext context, IWebHostEnvironment env, UserManager<IdentityUser> userManager)
        {
            _context = context;
            _env = env;
        ...
        // === CREATE ===
        [HttpPost]
        public async Task<IActionResult> CreateOperacoesPost([FromForm] PostCreateDto dto)
        {
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canCreate = User.HasClaim("OpCreatePost", "true");
            ...
            _context.OperacoesPosts.Add(post);
        ...
        // === GET ===
        [HttpGet]
        public async Task<IActionResult> GetOperacoesPosts([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
        {
            var user = await _userManager.GetUserAsync(User);
            ...
            var query = _context.OperacoesPosts
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateOperacoesPost(int id, [FromBody] PostUpdateDto dto)
        {
            var post = await _context.OperacoesPosts.FindAsync(id);
            ...
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canUpdate = User.HasClaim("OpUpdatePost", "true");
            ...
            return Ok(new { message = "OperacoesPost atualizado com sucesso.", post });
        }

        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteOperacoesPost(int id)
        {
            var post = await _context.OperacoesPosts.FindAsync(id);
            if (post == null) return NotFound();
            ...
            var isSupervisor = author?.SupervisorId == user.Id;
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canDelete = User.HasClaim("OpDeletePost", "true");
            ...
            _context.OperacoesPosts.Remove(post);
            await _context.SaveChangesAsync();

            return Ok(new { message = "OperacoesPost removido com sucesso." });
        }

        [HttpGet("orientador/buttons")]
        public async Task<IActionResult> GetButtons()
        {
            var buttons = await _context.OperacoesOrientadorButtons
                .OrderBy(b => b.Order)
                .ToListAsync();
            return Ok(buttons);
        }
        ...
        {
            if (!User.HasClaim("OpCreateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var maxOrder = await _context.OperacoesOrientadorButtons.MaxAsync(b => (int?)b.Order) ?? 0;

            var button = new OperacoesOrientadorButton
            {
                Text = dto.Text,
                Color = dto.Color,
                Bold = dto.Bold,
            ...
            _context.OperacoesOrientadorButtons.Add(button);
            await _context.SaveChangesAsync();
        ...
        {
            if (!User.HasClaim("OpUpdateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.OperacoesOrientadorButtons.FindAsync(id);
        ...
        {
            if (!User.HasClaim("OpDeleteOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.OperacoesOrientadorButtons.FindAsync(id);
            if (button == null) return NotFound();

            _context.OperacoesOrientadorButtons.Remove(button);
            await _context.SaveChangesAsync();
            return Ok();
        }
        ...
        public async Task<IActionResult> Reorder([FromBody] List<int> orderedIds)
        {
            ...
            var buttons = await _context.OperacoesOrientadorButtons.ToListAsync();
        ...
        {
            var table = await _context.OperacoesOrientadorTables
        ...
        {
            if (!User.HasClaim("OpCreateOri", "true") &&
                !User.HasClaim("OpUpdateOri", "true") &&
                !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var existing = await _context.OperacoesOrientadorTables
                .FirstOrDefaultAsync(t => t.ButtonId == buttonId);

            if (existing == null)
            {
                var newTable = new OperacoesOrientadorTable
                {
                    ButtonId = buttonId,
                    DataJson = dto.DataJson
                };
                _context.OperacoesOrientadorTables.Add(newTable);
            }
            else
            {
        ...
        [HttpGet("organograma")]
        public async Task<IActionResult> GetOperacoesOrganograma()
        {
            var allUsers = await _userManager.Users.ToListAsync();
            var gerentesOperacoes = new List<object>();

            foreach (var user in allUsers)
            {
                var userClaims = await _userManager.GetClaimsAsync(user);

                if (userClaims.Any(c => c.Type == "GerenteOperacoes" && c.Value == "true"))
                {
                    var colaborador = await _context.Colaboradores
                        .FirstOrDefaultAsync(c => c.Email.ToLower() == user.Email.ToLower());

                    // ⚡ Gerente: atividades, tags, historico
                    var gerenteAtividades = await _context.OperacoesColaboradorAtividades
                    ...
                    var subordinados = rawSubordinados
                        .Select(c =>
                        {
                            ...
                            var atividades = _context.OperacoesColaboradorAtividades
            ...
            return Ok(gerentesOperacoes);
        }


        [HttpPost("colaboradores/atividades")]
        public async Task<IActionResult> CriarAtividade([FromBody] OperacoesColaboradorAtividade dto)
        {
            var user = await _userManager.GetUserAsync(User);
            var hasPower = User.HasClaim("GerenteOperacoes", "true") || User.HasClaim("CanManageAll", "true");
            ...
            _context.OperacoesColaboradorAtividades.Add(dto);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> AtualizarAtividade(int id, [FromBody] OperacoesColaboradorAtividade update)
        {
            var atividade = await _context.OperacoesColaboradorAtividades.FindAsync(id);
        ...
        public async Task<IActionResult> DeletarAtividade(int id)
        {
            ...
            _context.OperacoesColaboradorAtividades.Remove(atividade);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> ListarAtividadesPorColaborador(string email)
        {
            ...
            var atividades = await _context.OperacoesColaboradorAtividades
