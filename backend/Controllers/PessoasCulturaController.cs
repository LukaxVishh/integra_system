...
namespace backend.Controllers
{
    ...
    [ApiController]
    public class PessoasCulturaController : Controller
    {
        private readonly AppDbContext _context;
        private readonly IWebHostEnvironment _env;
        private readonly UserManager<IdentityUser> _userManager;

        public PessoasCulturaController(AppDbContext context, IWebHostEnvironment env, UserManager<IdentityUser> userManager)
        {
            _context = context;
            _env = env;
        ...
        // === CREATE ===
        [HttpPost]
        public async Task<IActionResult> CreatePessoasCulturaPost([FromForm] PostCreateDto dto)
        {
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canCreate = User.HasClaim("PcCreatePost", "true");
            ...
            var post = new PessoasCulturaPost
            {
                AuthorId = user.Id,
            ...
            _context.PessoasCulturaPosts.Add(post);
        ...
        // === GET ===
        [HttpGet]
        public async Task<IActionResult> GetPessoasCulturaPosts([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
        {
            var user = await _userManager.GetUserAsync(User);
            ...
            var query = _context.PessoasCulturaPosts
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdatePessoasCulturaPost(int id, [FromBody] PostUpdateDto dto)
        {
            var post = await _context.PessoasCulturaPosts.FindAsync(id);
            ...
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canUpdate = User.HasClaim("PcUpdatePost", "true");
            ...
            return Ok(new { message = "PessoasCulturaPost atualizado com sucesso.", post });
        }

        [HttpDelete("{id}")]
        public async Task<IActionResult> DeletePessoasCulturaPost(int id)
        {
            var post = await _context.PessoasCulturaPosts.FindAsync(id);
            if (post == null) return NotFound();
            ...
            var isSupervisor = author?.SupervisorId == user.Id;
            var canManageAll = User.HasClaim("CanManageAll", "true");
            var canDelete = User.HasClaim("PcDeletePost", "true");
            ...
            _context.PessoasCulturaPosts.Remove(post);
            await _context.SaveChangesAsync();

            return Ok(new { message = "PessoasCulturaPost removido com sucesso." });
        }

        [HttpGet("orientador/buttons")]
        public async Task<IActionResult> GetButtons()
        {
            var buttons = await _context.PessoasCulturaOrientadorButtons
                .OrderBy(b => b.Order)
                .ToListAsync();
            return Ok(buttons);
        }
        ...
        {
            if (!User.HasClaim("PcCreateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var maxOrder = await _context.PessoasCulturaOrientadorButtons.MaxAsync(b => (int?)b.Order) ?? 0;

            var button = new PessoasCulturaOrientadorButton
            {
                Text = dto.Text,
                Color = dto.Color,
                Bold = dto.Bold,
            ...
            _context.PessoasCulturaOrientadorButtons.Add(button);
            await _context.SaveChangesAsync();
        ...
        {
            if (!User.HasClaim("PcUpdateOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.PessoasCulturaOrientadorButtons.FindAsync(id);
        ...
        {
            if (!User.HasClaim("PcDeleteOri", "true") && !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var button = await _context.PessoasCulturaOrientadorButtons.FindAsync(id);
            if (button == null) return NotFound();

            _context.PessoasCulturaOrientadorButtons.Remove(button);
            await _context.SaveChangesAsync();
            return Ok();
        }
        ...
        public async Task<IActionResult> Reorder([FromBody] List<int> orderedIds)
        {
            ...
            var buttons = await _context.PessoasCulturaOrientadorButtons.ToListAsync();
        ...
        {
            var table = await _context.PessoasCulturaOrientadorTables
        ...
        {
            if (!User.HasClaim("PcCreateOri", "true") &&
                !User.HasClaim("PcUpdateOri", "true") &&
                !User.HasClaim("CanManageAll", "true"))
                return Forbid();

            var existing = await _context.PessoasCulturaOrientadorTables
                .FirstOrDefaultAsync(t => t.ButtonId == buttonId);

            if (existing == null)
            {
                var newTable = new PessoasCulturaOrientadorTable
                {
                    ButtonId = buttonId,
                    DataJson = dto.DataJson
                };
                _context.PessoasCulturaOrientadorTables.Add(newTable);
            }
            else
            {
        ...
        [HttpGet("organograma")]
        public async Task<IActionResult> GetPessoasCulturaOrganograma()
        {
            var allUsers = await _userManager.Users.ToListAsync();
            var gerentesPessoasCultura = new List<object>();

            foreach (var user in allUsers)
            {
                var userClaims = await _userManager.GetClaimsAsync(user);

                if (userClaims.Any(c => c.Type == "GerentePessoasCultura" && c.Value == "true"))
                {
                    var colaborador = await _context.Colaboradores
                        .FirstOrDefaultAsync(c => c.Email.ToLower() == user.Email.ToLower());

                    // ⚡ Gerente: atividades, tags, historico
                    var gerenteAtividades = await _context.PessoasCulturaColaboradorAtividades
                    ...
                    var subordinados = rawSubordinados
                        .Select(c =>
                        {
                            ...
                            var atividades = _context.PessoasCulturaColaboradorAtividades
                    ...
                    gerentesPessoasCultura.Add(new
                    {
                        id = user.Id,
            ...
            return Ok(gerentesPessoasCultura);
        }


        [HttpPost("colaboradores/atividades")]
        public async Task<IActionResult> CriarAtividade([FromBody] PessoasCulturaColaboradorAtividade dto)
        {
            var user = await _userManager.GetUserAsync(User);
            var hasPower = User.HasClaim("GerentePessoasCultura", "true") || User.HasClaim("CanManageAll", "true");
            ...
            _context.PessoasCulturaColaboradorAtividades.Add(dto);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> AtualizarAtividade(int id, [FromBody] PessoasCulturaColaboradorAtividade update)
        {
            var atividade = await _context.PessoasCulturaColaboradorAtividades.FindAsync(id);
        ...
        public async Task<IActionResult> DeletarAtividade(int id)
        {
            ...
            _context.PessoasCulturaColaboradorAtividades.Remove(atividade);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> ListarAtividadesPorColaborador(string email)
        {
            ...
            var atividades = await _context.PessoasCulturaColaboradorAtividades
